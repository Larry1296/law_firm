  ...
  return <div className={compact ? 'space-y-4' : 'space-y-6 p-4 md:p-6'}>
    ...
    <Card className='p-5'>
      ...
      <div className='mt-3 grid gap-3 md:grid-cols-3'>
        ...
        <select className='rounded-xl border p-3 dark:bg-background-dark' value={reference.document_id} onChange={(e) => setReference({ ...reference, document_id: e.target.value })}><option value=''>Choose matching document</option>{selectableDocuments.map((item) => <option key={item.id} value={item.id}>{item.reference} — {item.title} — {item.subtype_label} — {item.verification_status.replaceAll('_', ' ')} — {item.source_copy_type.replaceAll('_', ' ')}{item.physical_storage_location ? ` — ${item.physical_storage_location}` : ''}</option>)}</select>
    ...
    <Card className='p-5'><h3 className='font-semibold'>Matter documents and client KYC file</h3>{isLoading && <p>Loading file…</p>}<div className='mt-4 grid gap-5 lg:grid-cols-3'><div><h4 className='text-sm font-bold uppercase tracking-wide'>Referenced client documents</h4><div className='mt-2 space-y-2'>{matterDocuments.map((item) => <button type='button' onClick={() => downloadDocument(item)} key={item.id} className='flex w-full gap-3 border-b p-3 text-left'><FileText/><span><strong>{item.reference} — {item.title}</strong><small className='block'>{item.subtype_label} · {item.verification_status.replaceAll('_', ' ')} · {item.physical_storage_location}</small></span></button>)}{!matterDocuments.length && <p className='text-sm text-text-muted-light dark:text-text-muted-dark'>No client documents referenced.</p>}</div></div><div><h4 className='text-sm font-bold uppercase tracking-wide'>Matter work product / court documents</h4><div className='mt-2 space-y-2'>{generatedMatterDocuments.map((item) => <div key={item.id} className='border-b p-3'><strong>{item.document_reference} — {item.title}</strong><small className='block'>{item.attachment_type_label} · {item.version_count} version(s)</small></div>)}{!generatedMatterDocuments.length && <p className='text-sm text-text-muted-light dark:text-text-muted-dark'>No matter work product recorded.</p>}</div></div><div><h4 className='text-sm font-bold uppercase tracking-wide'>Available client/KYC register</h4><p className='text-xs text-text-muted-light dark:text-text-muted-dark'>Client-file records not yet referenced to this matter.</p><div className='mt-2 space-y-2'>{clientFileDocuments.map((item) => <button type='button' onClick={() => downloadDocument(item)} key={item.id} className='flex w-full gap-3 border-b p-3 text-left'><FileText/><span><strong>{item.reference} — {item.title}</strong><small className='block'>{item.subtype_label} · {item.physical_storage_location}</small></span></button>)}{!clientFileDocuments.length && <p className='text-sm text-text-muted-light dark:text-text-muted-dark'>No unlinked client/KYC documents.</p>}</div></div></div></Card>
  </div>;
}
