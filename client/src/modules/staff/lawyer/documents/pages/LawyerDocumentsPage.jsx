...
export default function LawyerDocumentsPage({ caseId = '', compact = false }) {
  ...
  return <div className={compact ? 'space-y-4' : 'space-y-6 p-4 md:p-6'}>
    ...
    <Card className='p-5'>
      ...
      {selectedCase && <div className='mt-3 grid gap-2 rounded-xl border p-4 text-sm md:grid-cols-3'><p><strong>Matter:</strong><br />{selectedCase.case_number}</p><p><strong>Client:</strong><br />{selectedCase.client_name}</p><p><strong>KYC file:</strong><br />{selectedCase.kyc_drawer_reference || 'Not assigned'}</p></div>}
      <div className='mt-3 grid gap-3 md:grid-cols-3'>
        ...
        <select aria-label='Referenced physical document' className='rounded-xl border p-3 dark:bg-background-dark' value={reference.document_id} onChange={(e) => setReference({ ...reference, document_id: e.target.value })}><option value=''>Choose matching document</option>{selectableDocuments.map((item) => <option key={item.id} value={item.id}>{item.reference} — {item.subtype_label}{item.document_owner_subject ? ` — ${item.document_owner_subject}` : ''}{item.document_identifier ? ` — ${item.document_identifier}` : ''} — {item.source_copy_type_label || item.source_copy_type.replaceAll('_', ' ')}{item.page_count ? ` — ${item.page_count} pages` : ''}{item.verification_status ? ` — ${item.verification_status.replaceAll('_', ' ')}` : ''}{item.physical_storage_location ? ` — ${item.physical_storage_location}` : ''}</option>)}</select>
