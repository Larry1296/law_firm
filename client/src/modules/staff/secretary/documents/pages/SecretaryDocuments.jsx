...
export default function SecretaryDocuments({ caseId = '', compact = false }) {
  ...
  const [form, setForm] = useState({ client_id: '', case_id: caseId, request_id: '', title: '', document_reference: '', received_from: '', document_type: 'EVIDENCE', purpose: 'CLIENT_INSTRUCTION', description: '', received_via: 'IN_PERSON', physical_storage_location: '', custody_notes: '' });
  ...
  const assignDrawer = useMutation({
    mutationFn: () => secretaryDocumentsService.assignDrawer({ client_id: effectiveClientId, kyc_drawer_reference: drawerReference }),
    onSuccess: () => { setDrawerReference(''); queryClient.invalidateQueries({ queryKey: ['secretary-documents'] }); },
  });
  ...
  return <div className={compact ? 'space-y-4' : 'space-y-6 p-4 md:p-6'}>
    ...
    <Card className='p-5'>
      ...
      <div className='mt-4 grid gap-3 md:grid-cols-2'>
        ...
        <input className='rounded-xl border p-3 dark:bg-background-dark' placeholder='Exact physical document reference *' value={form.document_reference} onChange={(e) => setForm({ ...form, document_reference: e.target.value.toUpperCase() })}/>
