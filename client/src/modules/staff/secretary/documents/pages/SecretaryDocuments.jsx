...
export default function SecretaryDocuments({ caseId = '', compact = false }) {
  ...
  const registerPhysical = useMutation({
    mutationFn: () => secretaryDocumentsService.registerPhysicalDocument({ ...form, client_id: effectiveClientId }),
  ...
  return <div className={compact ? 'space-y-4' : 'space-y-6 p-4 md:p-6'}>
    ...
    <Card className='p-5'>
      <h3 className='font-semibold'>Physical KYC drawer assignment</h3>
      <p className='mt-1 text-sm'>Current drawer: <strong>{selectedClient?.kyc_drawer_reference || 'Not assigned'}</strong></p>
      <div className='mt-3 flex flex-col gap-3 sm:flex-row'>
        <input className='flex-1 border-b p-3 dark:bg-background-dark' placeholder='KYC-2026-039' value={drawerReference} onChange={(event) => setDrawerReference(event.target.value.toUpperCase())}/>
        <input className='flex-1 border-b p-3 dark:bg-background-dark' placeholder='Cabinet A / Drawer 3' value={cabinetLocation} onChange={(event) => setCabinetLocation(event.target.value)}/>
        <button type='button' className='bg-brand-primary px-4 py-2 font-semibold text-white disabled:opacity-50' disabled={!effectiveClientId || !drawerReference || assignDrawer.isPending} onClick={() => assignDrawer.mutate()}>{assignDrawer.isPending ? 'Assigning…' : 'Assign drawer'}</button>
      </div>
    ...
    <Card className='p-5'>
      <h3 className='font-semibold'>Register a physical document in the KYC drawer</h3>
      <p className='mt-1 text-sm text-text-muted-light dark:text-text-muted-dark'>No file is uploaded. Record the received physical document against the client’s permanent KYC drawer reference and, where applicable, the advocate’s request.</p>
      <div className='mt-4 grid gap-3 md:grid-cols-2'>
        ...
        <div className='flex gap-2'><input className='min-w-0 flex-1 border-b p-3 dark:bg-background-dark' placeholder='Exact physical document reference *' value={form.document_reference} onChange={(e) => setForm({ ...form, document_reference: e.target.value.toUpperCase() })}/><button type='button' className='border-b px-3 text-sm font-semibold' disabled={proposeReference.isPending} onClick={() => proposeReference.mutate()}>Propose next</button></div>
        <input className='rounded-xl border p-3 dark:bg-background-dark' placeholder='Received from — e.g. Mutiso *' value={form.received_from} onChange={(e) => setForm({ ...form, received_from: e.target.value })}/>
        <label className='text-sm font-semibold'>Exact date and time received<input className='mt-1 w-full border-b p-3 font-normal dark:bg-background-dark' type='datetime-local' value={form.received_at} onChange={(e) => setForm({ ...form, received_at: e.target.value })}/></label>
        ...
        <select className='border-b p-3 dark:bg-background-dark' value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}><option value='KYC_IDENTITY'>KYC / identity</option><option value='KYC_TAX'>KYC / tax identification</option><option value='ENTITY_RECORD'>Entity / authority record</option><option value='PROPERTY'>Property / land</option><option value='TRANSACTION'>Transaction / commercial</option><option value='MATTER_EVIDENCE'>Matter evidence</option><option value='CORRESPONDENCE'>Correspondence</option><option value='OTHER'>Other</option></select>
        <select className='border-b p-3 dark:bg-background-dark' value={form.subtype} onChange={(e) => setForm({ ...form, subtype: e.target.value })}><option value='NATIONAL_ID'>National ID</option><option value='PASSPORT'>Passport</option><option value='ALIEN_ID'>Alien ID / foreign national certificate</option><option value='KRA_PIN'>KRA PIN Certificate</option><option value='INCORPORATION'>Certificate of incorporation / registration</option><option value='CR12'>CR12 / company search</option><option value='TITLE_DEED'>Title Deed</option><option value='OFFICIAL_SEARCH'>Official Search</option><option value='CONTRACT'>Contract / Agreement</option><option value='INVOICE'>Invoice</option><option value='RECEIPT'>Receipt</option><option value='DELIVERY_NOTE'>Delivery Note</option><option value='CORRESPONDENCE'>Correspondence</option><option value='POLICE_ABSTRACT'>Police Abstract</option><option value='OTHER'>Other</option></select>
        ...
        <input className='rounded-xl border p-3 dark:bg-background-dark' placeholder='KYC drawer / physical file location (required)' value={form.physical_storage_location} onChange={(e) => setForm({ ...form, physical_storage_location: e.target.value })}/>
      ...
      {registerPhysical.isSuccess && <p className='mt-3 text-success'>Physical document recorded under the KYC drawer reference. The advocate has been notified if this fulfilled a request.</p>}
      <button className='mt-4 rounded-xl bg-brand-primary px-4 py-2 text-white disabled:opacity-50' disabled={!form.title || !form.document_reference || !form.received_from || !effectiveClientId || !selectedClient?.kyc_drawer_reference || registerPhysical.isPending} onClick={() => registerPhysical.mutate()}>{registerPhysical.isPending ? 'Recording…' : 'Record physical document receipt'}</button>
    </Card>
