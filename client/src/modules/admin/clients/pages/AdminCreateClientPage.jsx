    ...
    {!metadata ? <p>Loading client categories…</p> : <form onSubmit={submit} className='space-y-5'>
      ...
      {schema && <>
        ...
        <StepPanel title='Privacy notice delivery'><div className='grid gap-4 sm:grid-cols-2'>
          {!metadata.intake_privacy && <p role='alert' className='rounded-lg bg-amber-50 p-3 text-amber-900 sm:col-span-2'>An approved active firm intake-privacy configuration is required before creation. Ask your firm administrator to configure and approve it. Lawful basis and privacy notice version are filled automatically and cannot be typed here.</p>}
          <label className='block text-sm text-[color:var(--text-primary)]'><span className='mb-1 block font-medium'>Lawful basis</span><input className='w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2 text-[color:var(--text-primary)] placeholder:text-[color:var(--text-secondary)]' placeholder='No approved configuration' readOnly value={metadata.intake_privacy?.lawful_basis_label || ''} /></label>
          <label className='block text-sm text-[color:var(--text-primary)]'><span className='mb-1 block font-medium'>Privacy notice version</span><input className='w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2 text-[color:var(--text-primary)] placeholder:text-[color:var(--text-secondary)]' placeholder='No approved configuration' readOnly value={metadata.intake_privacy?.policy_version || ''} /></label>
