...
import { buildOnboardingPayload, onboardingReducer, onboardingStateFromSearch } from '../clientCreation/onboardingState';
...
export default function AdminCreateClientPage() {
  ...
  const [state,dispatch]=useReducer(onboardingReducer,location.search,onboardingStateFromSearch); const [metadata,setMetadata]=useState(null); const [step,setStep]=useState(0); const [error,setError]=useState(''); const [saving,setSaving]=useState(false); const [success,setSuccess]=useState(null);
  ...
  const submit=async()=>{
    setError('');
    if(state.client.access_type==='PORTAL_ENABLED'&&!state.representatives.some((item)=>item.is_portal_contact)){
      setError('Portal-enabled companies require an authorised representative to be selected as the company portal contact. Complete that selection under Representatives.');
      setStep(3);
      return;
    }
    setSaving(true);
    try{const result=await service.createOnboardingClient(buildOnboardingPayload(state));setSuccess(result);}
    catch(e){setError(e.response?.data?.detail||e.response?.data?.message||e.message||'Unable to create the client.');}
