...
export function Field({ label, value = '', onChange, type = 'text', required = false, help = '' }) {
  ...
  return <label className='block text-sm text-[color:var(--text-primary)]'>
    ...
    <input type={type} required={required} value={value ?? ''} onChange={(e) => onChange(e.target.value)} className='w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--surface)] px-3 py-2' />
