...
class SecretaryDocumentService:
    ...
    def verify_client_upload(user, request_id, data):
        ...
        if document.physical_copy_retained and not document.physical_storage_location:
            raise ValidationError({"physical_storage_location": "Record the KYC drawer or physical file location."})
    ...
    def workspace(user, params):
        ...
        return {
            ...
            "clients": [{"id": str(item.id), "name": item.full_name} for item in clients],
